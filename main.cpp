...
int main(int argc, char **argv) {
	...
	//4x multisampling, so the circles and slanted edges are not jagged (cheap for a 2D scene):
	...
	Mode::window = create_window();
	if (!Mode::window) { //no multisampled framebuffer available (e.g. software rendering)? try again without:
	...
	//------------ init sound --------------
	//ask for a small device buffer (256 frames = 5 ms at 48 kHz) so key presses are answered quickly; SDL's default is 1024:
